...
public class ConductorServer {
	...
	public synchronized void start(int port, boolean join) throws Exception {
		...
		try {
			...
			if(create) {
				System.out.println("Creating kitchensink and Timeseries workflow");
				createKitchenSink(port);
				createTimeseriesWorkflow(port);
			}
		}catch(Exception e) {
	...
	private static void createKitchenSink(int port) throws Exception {
		...
		private static void createTimeseriesWorkflow(int port) throws Exception {
		
		Client client = Client.create();
		InputStream stream = Main.class.getResourceAsStream("/tasks.json");
		client.resource("http://localhost:" + port + "/api/metadata/taskdefs").type(MediaType.APPLICATION_JSON).post(stream);
		
		stream = Main.class.getResourceAsStream("/Timeseries.json");
		client.resource("http://localhost:" + port + "/api/metadata/workflow").type(MediaType.APPLICATION_JSON).post(stream);
		
		stream = Main.class.getResourceAsStream("/RemoveAndRescheduleTimeseries.json");
		client.resource("http://localhost:" + port + "/api/metadata/workflow").type(MediaType.APPLICATION_JSON).post(stream);
		
		stream = Main.class.getResourceAsStream("/Send2ECP.json");
		client.resource("http://localhost:" + port + "/api/metadata/workflow").type(MediaType.APPLICATION_JSON).post(stream);
		
		stream = Main.class.getResourceAsStream("/Timeseries2ECP_Failure.json");
		client.resource("http://localhost:" + port + "/api/metadata/workflow").type(MediaType.APPLICATION_JSON).post(stream);

		logger.info("Timeseries workflows are created!");
	}
}
